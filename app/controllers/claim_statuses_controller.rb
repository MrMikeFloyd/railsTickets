class ClaimStatusesController < ApplicationController
  before_action :set_claim_status, only: [:show, :edit, :update, :destroy]

  # GET /claim_statuses
  # GET /claim_statuses.json
  def index
    @claim_statuses = ClaimStatus.all
  end

  # GET /claim_statuses/1
  # GET /claim_statuses/1.json
  def show
  end
  ...
  # POST /claim_statuses
  # POST /claim_statuses.json
  def create
    @claim_status = ClaimStatus.new(claim_status_params)

    respond_to do |format|
      if @claim_status.save
        format.html { redirect_to @claim_status, notice: 'Claim status was successfully created.' }
  ...
  # PATCH/PUT /claim_statuses/1
  # PATCH/PUT /claim_statuses/1.json
  def update
    respond_to do |format|
      if @claim_status.update(claim_status_params)
        format.html { redirect_to @claim_status, notice: 'Claim status was successfully updated.' }
  ...
  private
    ...
    def claim_status_params
      params.require(:claim_status).permit(:name, :description)
    end
end
